...
interface ImboxLoadDataOfficialDutyResult {
  status: boolean;
  message: string;
  jenis: string;
  data: any[];
...
@Injectable()
export class ImboxLoadDataOfficialDutyService {
  constructor(private readonly dbService: DbService) {}

  async loadDataOfficialDuty(dto: {
    database: string;
    em_id: string;
    branch_id?: string;
    bulan?: string;
    ...
    end_periode: string;
  }): Promise<ImboxLoadDataOfficialDutyResult> {
    ...
    a.typeid,
     ...
     FROM ${startDb}.emp_leave a JOIN ${database}_hrm.employee b WHERE a.em_id=b.em_id AND  
    ...
    a.typeid,
     ...
     b.em_report_to as em_report_to,  b.em_report2_to as em_report2_to,   b.full_name  
     FROM ${endDb}.emp_leave a JOIN ${database}_hrm.employee b WHERE a.em_id=b.em_id AND  
     (b.em_report_to LIKE '%${em_id}%' OR b.em_report2_to LIKE '%${em_id}%')  ${conditionStatus}  
     AND a.leave_status!='Cancel' AND a.ajuan='4'  AND a.status_transaksi=1 ${orderby1}`;
