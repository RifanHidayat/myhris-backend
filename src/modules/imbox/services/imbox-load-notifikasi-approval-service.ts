...
interface ImboxLoadNotifikasiApprovalResult {
  status: boolean;
  message: string;
...
@Injectable()
export class ImboxLoadNotifikasiApprovalService {
  constructor(private readonly dbService: DbService) {}

  async load_notifikasiApproval(dto: {
    database: string;
    em_id: string;
    tahun: string;
    bulan: string;
    ...
    end_periode?: string;
  }): Promise<ImboxLoadNotifikasiApprovalResult> {
    ...
    let query1 = `SELECT atten_date FROM notifikasi WHERE em_id = '${em_id}' AND em_id_pengajuan != '${em_id}' AND idx IS NOT NULL ORDER BY id DESC`;
    let query2 = `SELECT * FROM notifikasi WHERE em_id = '${em_id}' AND em_id_pengajuan != '${em_id}' AND idx IS NOT NULL`;
