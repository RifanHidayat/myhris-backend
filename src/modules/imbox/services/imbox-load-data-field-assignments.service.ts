...
export class ImboxLoadDataFieldAssignmentsService {
  ...
  async loadDataFieldAssignments(dto: {
   ...
   a.approve_date,
   a.id,
     b.em_report_to as em_report_to,  b.em_report2_to as em_report2_to,   b.full_name FROM ${startDb}.emp_labor a JOIN ${database}_hrm.employee b WHERE a.em_id=b.em_id AND (b.em_report_to LIKE '%${em_id}%' OR b.em_report2_to LIKE '%${em_id}%') ${conditionStatus} AND a.status!='Cancel' AND a.ajuan='2'  AND a.status_transaksi=1 
     ${orderby1} 
     `;
    ...
    if (montStart < monthEnd || date1.getFullYear() < date2.getFullYear()) {
     ...
     a.approve_date,
     a.id,
       b.em_report_to as em_report_to,  b.em_report2_to as em_report2_to,   b.full_name FROM ${endDb}.emp_labor a JOIN ${database}_hrm.employee b WHERE a.em_id=b.em_id AND (b.em_report_to LIKE '%${em_id}%' OR b.em_report2_to LIKE '%${em_id}%') ${conditionStatus} AND a.status!='Cancel' AND a.ajuan='2'  AND a.status_transaksi=1 
       ${orderby1} 
       `;
    }
