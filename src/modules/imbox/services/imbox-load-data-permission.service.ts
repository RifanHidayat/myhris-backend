...
interface ImboxLoadDataPermissionResult {
  status: boolean;
  message: string;
  jenis: string;
  data: any[];
...
@Injectable()
export class ImboxLoadDataPermissionService {
  constructor(private readonly dbService: DbService) {}

  async loadDataPermission(dto: {
    database: string;
    em_id: string;
    branch_id?: string;
    bulan?: string;
    ...
    end_periode: string;
  }): Promise<ImboxLoadDataPermissionResult> {
...
a.time_plan_to,
a.time_plan,
     c.name as nama_penagjuan, b.em_report_to as em_report_to,  b.em_report2_to as em_report2_to,   b.full_name, c.name as nama_tipe, c.category FROM ${startDb}.emp_leave a INNER JOIN ${database}_hrm.leave_types c ON a.typeid=c.id JOIN ${database}_hrm.employee b WHERE a.em_id=b.em_id 
...
a.time_plan_to,
a.time_plan,
     c.name as nama_penagjuan, b.em_report_to as em_report_to,  b.em_report2_to as em_report2_to,   b.full_name, c.name as nama_tipe, c.category FROM ${endDb}.emp_leave a INNER JOIN ${database}_hrm.leave_types c ON a.typeid=c.id JOIN ${database}_hrm.employee b WHERE a.em_id=b.em_id 
    AND (b.em_report_to LIKE '%${em_id}%' OR b.em_report2_to LIKE '%${em_id}%')  ${conditionStatus}  AND a.leave_status!='Cancel' AND a.ajuan IN ('2', '3')  AND a.status_transaksi=1 ${orderby1}`;
;
    }
